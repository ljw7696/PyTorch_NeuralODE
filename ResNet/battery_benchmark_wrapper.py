...
class DNNBenchmark(nn.Module):
    ...
    Uses normalized values: V_ref, ocv, Y, I, T are all normalized.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: shape (..., input_dim) containing [V_ref, ocv, Y, SOC, I, T]
...
def _extract_features_and_targets(
    ...
    if any(arr is None for arr in (V_ref, ocv, soc, current, temperature, v_spme, v_meas)):
        missing = [
            name
            for name, arr in zip(
                ["V_ref/V_meas", "ocv", "SOC", "I", "T", "V_spme", "V_meas"],
                (V_ref, ocv, soc, current, temperature, v_spme, v_meas),
            )
            if arr is None
        ]
    ...
    min_len = min(
        len(V_ref), len(ocv), len(Y_norm), len(soc), len(current), len(temperature), len(v_spme), len(v_meas)
    )
    if min_len < 2:
...
class SingleStepDataset(Dataset):
    ...
    - Input features: normalized (V_ref, ocv, Y, SOC, I, T)
    ...
    def __init__(self, profiles: Sequence[dict]):
        ...
        for profile_idx, profile in enumerate(profiles):
            ...
            # features: [V_ref, ocv, Y, SOC, I, T] where Y is normalized
...
def train_dnn_benchmark(
    ...
    *,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # Use validation metrics for scheduler and model selection if available
...
def _dnn_autoregressive_rollout_train_batch(
    ...
    Args:
        ...
        features_batch: torch.Tensor shape [batch_size, T_max, 6] containing normalized values [V_ref, ocv, Y, SOC, I, T]
    ...
    pred_y_norm = torch.zeros(batch_size, T_max, dtype=torch.float32, device=device, requires_grad=False)
    
    # Initial condition: normalized Y at k=0 (only for valid profiles)
    pred_y_norm[:, 0] = features_batch[:, 0, 2]  # Y(k=0) normalized
    
    # Autoregressive rollout for each time step (ResNet style: Y(k+1) = Y(k) + dY)
    for k in range(T_max - 1):
        # Build input batch (create new tensor to avoid inplace operation issues with autograd)
        input_batch = torch.stack([
            features_batch[:, k, 0],  # V_ref (normalized)
...
def _dnn_autoregressive_rollout_train(
    ...
    Args:
        model: DNNBenchmark model
        features: torch.Tensor shape [T, 6] containing normalized values [V_ref, ocv, Y, SOC, I, T]
...
def _dnn_autoregressive_rollout(
    ...
    Args:
        ...
        features: np.ndarray shape [T, 6] as returned by _extract_features_and_targets
                 Contains normalized values: [V_ref, ocv, Y, SOC, I, T]
        device: torch device
