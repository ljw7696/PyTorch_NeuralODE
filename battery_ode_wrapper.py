...
def train_battery_neural_ode(data_dict, num_epochs=100, lr=1e-3, device='cpu', verbose=True, pretrained_model_path=None):
    ...
    for epoch in range(num_epochs):
        ...
        loss_dVdt = torch.mean((dVdt_pred - dVdt_ref ) ** 2)
        loss_V = torch.mean((Vcorr_pred - Vcorr_target) ** 2)
        loss_tv = torch.mean(torch.abs(torch.diff(Vcorr_pred)))
...
def simulate_battery_ode(ode_wrapper, data_dict, device='cuda', plot=True):
    ...
    if plot:
        ...
        if 'I' in data_dict:
            current = np.array(data_dict['I']) * 0.25  # De-normalize current
            fig.add_trace(
...
def analyze_feature_importance(ode_wrapper, data_dict, device='cuda', verbose=True):
    ...
    for t_val in t_sample:
        ...
        with torch.no_grad():
            ...
            if len(t_subset) > 0:
                with torch.no_grad():
                    solution_subset = odeint(ode_wrapper, x0_norm, t_subset, method='rk4')
